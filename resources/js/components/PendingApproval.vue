    ...
    <table class="table">
      ...
      <tbody class="table-border-bottom-0">
        <tr class="text-nowrap" v-for="company in companies" :key="company.id">
          ...
          <td><span class="me-1">{{ company.product }}</span></td>
          <td><span class="me-1">{{ company.email }}</span></td>
          <td><span class="me-1">{{ company.type }}</span></td>
          <td class="d-flex">
...
export default {
  ...
  setup(props) {
    ...
    const viewCompany = (company) => {
      window.location.href = base_url+props.bank+'/companies/pending/'+company.id+'/details'
    }
